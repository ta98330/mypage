...
if(!empty(@$_POST['come'] || @$_POST['out'])){
    ...
    if(!empty($_POST['come'])){
        ...
        $st = $pdo->query("UPDATE member SET situation = '登校' WHERE id = {$_SESSION['userId']}");//現状の更新      
        ...
        $st = $pdo->query("UPDATE member SET situation = '下校' WHERE id = {$_SESSION['userId']}");//現状の更新
